...
dtype = {'prognosis': str}
ds = pd.read_csv(r'Training.csv', dtype=dtype)
X=ds.iloc[:,:132]
ds = pd.read_csv('Training.csv', usecols=[132], dtype=dtype)
Y=ds
ds = pd.read_csv('Testing.csv', dtype=dtype)
x=ds.iloc[:,:132]
ds = pd.read_csv('Testing.csv', usecols=[132], dtype=dtype)
y=ds
...
if __name__ == "__main__" :
    ...
    print("Total no of unique diseases :",np.unique(Y))
    ls = list(X.columns)
    ls.sort()
    for _ in ls:
        print(_)
